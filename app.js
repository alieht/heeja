...
var SUPABASE_KEY="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJlenp5dHBkY29kcnRrdGV5d3lrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODQzNzI3NDUsImV4cCI6MjA5OTk0ODc0NX0.gWRwIkorB6iwQkZdYadPIU80J27NEO8V9alenYcgejo";
...
function admTab(t,btn){admCurTab=t;sbSync();var tabs=document.querySelectorAll(".admin-tab");for(var i=0;i<tabs.length;i++)tabs[i].classList.remove("active");if(btn)btn.classList.add("active");var c=document.getElementById("admContent");if(!c)return;
  if(t==="dashboard"){
    ...
    var h='<div class="export-bar"><h3 style="color:var(--gl)">📊 نمای کلی</h3><button class="btn btn-gold btn-sm" onclick="admExportCSV()">📥 export CSV</button></div>';
    ...
    c.innerHTML=h;}else if(t==="orders"){var oh=gO(),h="";if(!oh.length){h='<p class="empty-state">هیچ سفارشی ثبت نشده</p>'}else{h='<table class="admin-table"><thead><tr><th>شماره</th><th>کد</th><th>مشتری</th><th>تلفن</th><th>نوع</th><th>اقلام</th><th>مبلغ</th><th>وضعیت</th><th></th></tr></thead><tbody>';for(var i=oh.length-1;i>=0;i--){var o=oh[i];var itemNames=o.items.map(function(x){return x.name+(x.sauce?'+'+x.sauce:'')+(x.drink?'+'+x.drink:'')}).join('، ');h+='<tr><td style="font-weight:900;color:var(--gl)">#'+rn(o)+'</td><td style="font-size:.75rem">'+o.id+'</td><td>'+o.cust.name+'</td><td>'+o.cust.phone+'</td><td>'+(o.cust.delivery==="delivery"?'🛵 پیک':'🏃 حضوری')+'</td><td style="font-size:.78rem;max-width:200px">'+itemNames+'</td><td>'+fp(o.total)+'</td><td><select onchange="admChSt(\x27'+o.id+'\x27,this.value)" style="background:var(--inp);color:var(--t);border:1px solid var(--b);border-radius:6px;padding:3px 6px;font-size:.7rem"><option value="pending"'+(o.status==="pending"?' selected':'')+'>⏳ در انتظار</option><option value="preparing"'+(o.status==="preparing"?' selected':'')+'>👨‍🍳 آماده</option><option value="ready"'+(o.status==="ready"?' selected':'')+'>📦 بسته</option><option value="shipped"'+(o.status==="shipped"?' selected':'')+'>🛵 ارسال</option><option value="delivered"'+(o.status==="delivered"?' selected':'')+'>✅ تحویل</option></select></td><td><button class="btn btn-danger btn-sm" onclick="admDelOrd(\x27'+o.id+'\x27)">🗑</button></td></tr>'}h+='</tbody></table>'}c.innerHTML=h}
...
function admAddDeal(){var d=L(),n=document.getElementById("dlName").value.trim(),pct=parseInt(document.getElementById("dlPct").value)||0,op=parseInt(document.getElementById("dlPrice").value)||0,iid=document.getElementById("dlItemId").value;if(!n||!pct||!op){st("همه فیلدها الزامی است","error");return}d.deals.push({id:"d"+Date.now(),n:n,d:pct,op:op,np:Math.round(op*(1-pct/100)),itemId:iid||null});S(d);admTab("deals");st("تخفیف اضافه شد")}
function admDelDeal(id){var d=L();d.deals=d.deals.filter(function(x){return x.id!==id});S(d);admTab("deals")}
...
function admDelItem(ck,sk,id){var d=L();d.cats[ck].scs[sk].its=d.cats[ck].scs[sk].its.filter(function(i){return i.id!==id});S(d);admTab("items")}
function admAddSauce(){var d=L(),n=document.getElementById("scName").value.trim(),p=parseInt(document.getElementById("scPrice").value)||0;if(!n){st("نام الزامی است","error");return}d.sauces.push({id:"sc"+Date.now(),n:n,p:p});S(d);admTab("sauces");st("سس اضافه شد")}
function admDelSauce(id){var d=L();d.sauces=d.sauces.filter(function(s){return s.id!==id});S(d);admTab("sauces")}
function admAddDrink(){var d=L(),n=document.getElementById("drName").value.trim(),p=parseInt(document.getElementById("drPrice").value)||0;if(!n){st("نام الزامی است","error");return}d.drinks.push({id:"dr"+Date.now(),n:n,p:p});S(d);admTab("drinks");st("نوشیدنی اضافه شد")}
function admDelDrink(id){var d=L();d.drinks=d.drinks.filter(function(x){return x.id!==id});S(d);admTab("drinks")}
...
function admUpdatePrice(ck,sk,id,val){var d=L();var its=d.cats[ck].scs[sk].its;for(var i=0;i<its.length;i++){if(its[i].id===id){its[i].p=parseInt(val)||0;break}}S(d);st('✅ قیمت بروز شد')}
